...
# 역할 추출 => 프롬프트 생성 => 스타일 적용 => 이미지 생성 => 저장
...
class StoryDiffusionSynthesizer:
    def __init__(self,
                 ...
                 num_steps: int = 50,
                 cache_dir: str = "./model"):
        # self-attention 동기화를 위한 전역 인자 저장
        self.attn_args = {
            "attn_count": 0,     # 현재 attention 호출 횟수
            "cur_step": 0,       # 현재 diffusion step
            "total_count": 0     # 전체 attention processor 개수
        }

        # 기본 설정 값들 저장
        self.cache_dir = cache_dir
        self.sa32 = 0.5              # 32해상도 attention mask 비율
        self.sa64 = 0.5              # 64해상도 attention mask 비율
        self.id_length = id_length   # ID 이미지 개수 (일관성 유지용)
        ...
        # 프롬프트 스타일 정의 (긍정 프롬프트, 부정 프롬프트)
        self.styles = {
            '(No style)': ('{prompt}', ''),
            'Japanese Anime': (
                'anime artwork illustrating {prompt}... (Anime Style, Manga Style:1.3)...',
                'lowres, bad anatomy, bad hands, ...'),
            'Digital/Oil Painting': (
                '{prompt}. (Extremely Detailed Oil Painting:1.2)...',
                'anime, cartoon, glitch, deformed, ...'),
            'Pixar/Disney Character': (
                'Create a Disney Pixar 3D style illustration on {prompt}...',
                'lowres, bad anatomy, blurry, grainy, ...'),
            'Photographic': (
                'cinematic photo {prompt}. Hyperrealistic...',
                'drawing, painting, sketch, ...'),
            'Comic book': (
                'comic {prompt}. graphic illustration...',
                'photograph, realistic, glitch, ...'),
            'Line art': (
                'line art drawing {prompt}...',
                'photorealistic, 35mm film, blurry...'),
            'Black and White Film Noir': (
                '{prompt}. (b&w, Monochromatic, Film Photography:1.3)...',
                'realistic, blurry, sketch, glitch...'),
            'Isometric Rooms': (
                'Tiny cute isometric {prompt}...',
                'photo, realistic, anime...'),
            'Storybook': (
                "Cartoon style, cute illustration of {prompt}.",
                'photo, realism, blurry, worst quality...')
        }
        ...
        # U-Net에서 각 attention layer에 custom processor 삽입
        unet = pipe.unet
        attn_procs = {}

        for name in unet.attn_processors.keys():
            # cross attention 여부 확인
            ...
            # self-attention이면 SpatialAttnProcessor2_0 사용
        ...
        print("✅ self-attention processor 로드 완료")
        print(f"🧠 processor 수: {self.attn_args['total_count']}개")

        # attention processor 등록
    ...
    # write 모드 설정 (True면 ID 이미지 생성, False면 일반 이미지 생성)
    ...
    def call(self,
        ...
        assert len(prompts) == self.total_length, "프롬프트 수는 페이지 수와 같아야 합니다."

        setup_seed(seed)
        generator = torch.Generator(device=self.device).manual_seed(seed)
        torch.cuda.empty_cache()

        # 1. ID 프롬프트 → 일관성 유지용 이미지 생성
...
# StoryDiffusionAgent는 이야기(story) 텍스트를 바탕으로 이미지 프롬프트를 생성하고,
# Stable Diffusion 기반 모델을 통해 이미지를 생성하는 에이전트입니다.
...
class StoryDiffusionAgent:
    ...
    def call(self, params: Dict): # 스토리 페이지 및 저장 경로 불러오기
        pages: List = params["pages"]
        save_path: str = params["save_path"]
        role_dict = self.extract_role_from_story(pages) # 1. 이야기에서 등장인물(role)을 추출하여 이름과 설명 매핑
        image_prompts = self.generate_image_prompt_from_story(pages) # 2. 각 페이지에서 이미지 프롬프트 생성
        image_prompts_with_role_desc = [] # 3. 프롬프트 내 등장인물 이름을 해당 인물 설명으로 치환
        for image_prompt in image_prompts: # 4. 이미지 생성기 초기화
    ...
    def extract_role_from_story(
        ...
        role_reviewer = init_tool_instance({ # 역할 검토 에이전트 초기화
            "tool": self.cfg.get("llm", "qwen"),
            "cfg": {
